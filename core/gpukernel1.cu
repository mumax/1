...
void gpu_init_Greens_kernel1(tensor* dev_kernel, int N0, int N1, int N2, int *zero_pad, int *repetition, float *FD_cell_size){
	...
	// check input __________________________________________________________________________________
		assert(N0>0);
		assert(N1>0);
		assert(N2>0);
		for (int i=0; i<3; i++){
		...
		assert(FD_cell_size[X]>0.0f);
		assert(FD_cell_size[Y]>0.0f);
		assert(FD_cell_size[Z]>0.0f);
	// ______________________________________________________________________________________________


	// allocate tensor dev_kernel ___________________________________________________________________
...
void gpu_init_and_FFT_Greens_kernel_elements(tensor *dev_kernel, int *Nkernel, float *FD_cell_size, float cst, int *repetition, float *dev_qd_P_10, float *dev_qd_W_10, gpu_plan3d_real_input* kernel_plan){
	...
	// Define gpugrids and blocks ___________________________________________________________________
		dim3 gridsize1(Nkernel[X]/2, Nkernel[Y]/2, 1);	///@todo generalize!
		dim3 blocksize1(Nkernel[Z]/2, 1, 1);				    ///@todo aan te passen!!  GPU_STRIDE_FLOAT
