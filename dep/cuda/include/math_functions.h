...
/*******************************************************************************
*                                                                              *
* HOST IMPLEMENTATIONS FOR FUNCTIONS WITH BUILTIN NVOPENCC OPERATIONS          *
*                                                                              *
*******************************************************************************/
...
__device_func__(long long int __cuda_llmax(long long int a, long long int b))
{
#if defined(__CUDABE__)
  return llmax(a, b);
#else
...
__device_func__(long long int __cuda_llmin(long long int a, long long int b))
{
#if defined(__CUDABE__)
  return llmin(a, b);
#else
...
__device_func__(unsigned long long int __cuda_ullmax(unsigned long long int a, unsigned long long int b))
{
#if defined(__CUDABE__)
  return ullmax(a, b);
#else
...
__device_func__(unsigned long long int __cuda_ullmin(unsigned long long int a, unsigned long long int b))
{
#if defined(__CUDABE__)
  return ullmin(a, b);
#else
...
__device_func__(float __cuda_nextafterf(float a, float b))
{
  ...
  ia = __float_as_int(a);
  ib = __float_as_int(b);
#if defined(__CUDABE__) && defined(__CUDA_FTZ)
...
__device_func__(unsigned long long int __internal_nan_kernel(const char *s))
{
...
#else /* !__CUDABE__ || __CUDA_ARCH__ >= 200 */
  return a * b + c;
...
__device_func__(float __cuda_sinpif(float a))
{
  float z;
  int   i;
...
#if !defined(__CUDABE__) && defined(__GNUC__)
  /* Some versions of gcc don't produce INDEFINITE when argument is < -1.0 */
  if (a < -1.0f) return __int_as_float(0xffc00000);
#endif /* !__CUDABE__ && __GNUC__ */
  if (a >= -0.394f && a <= 0.65f) {
...
#if defined(__CUDABE__)
  if (__cuda_fabsf(t) <= CUDART_INF_F)
#endif
  {
    t = __int_as_float(__float_as_int(t) | (__float_as_int(a) & 0x80000000));
  }
...
#if !defined(__CUDABE__)
  /* Some host compilers don't handle NaNs correctly */
  if (__cuda___isnanf(a)) return a + a;
#endif
  s = __cuda_fabsf(a);
  t = __cuda_exp2f(CUDART_THIRD_F * __log2f(s));           /* initial approx */
...
__device_func__(float __cuda_rcbrtf(float a))
{
  float s, t;
...
#else /* !defined(__CUDABE__) || !defined(__CUDA_FTZ) */
    float scaled_b = __int_as_float ((__float_as_int(b) & 0x007fffff) | 
                                     (__float_as_int(a) & 0x7f800000));
    if (scaled_b > a) {
      scaled_b *= 0.5f;
    }
#endif /* !defined(__CUDABE__) || !defined(__CUDA_FTZ) */
    while (scaled_b >= b) {
      if (a >= scaled_b) {
        a -= scaled_b;
      }
...
__device_func__(float __cuda_tgammaf(float a))
{
  ...
  if (x >= 0.0f) {
    ...
    if (x == __cuda_floorf(x)) {  /* x is negative integer */
      x = __cuda_rsqrtf (__int_as_float (0xffc00000));  /* NaN */
#if !defined(__CUDABE__)
      return x;
#endif /* !__CUDABE__ */
    } 
...
#else /* __USE_FAST_MATH__ */
  float t, s, c;
  volatile float t2, u2; // HACK: use volatile, compiler issues (bug 588812)
  int quadrant;
  t = __cuda_fabsf(a);
...
/*******************************************************************************
*                                                                              *
* ONLY FOR HOST CODE! NOT FOR DEVICE EMULATION OR DEVICE EXECUTION             *
*                                                                              *
*******************************************************************************/

/*******************************************************************************
*                                                                              *
* HOST IMPLEMENTATION FOR DOUBLE AND FLOAT ROUTINES. WINDOWS & APPLE PLATFORMS *
*                                                                              *
*******************************************************************************/
...
/*******************************************************************************
*                                                                              *
* HOST IMPLEMENTATION FOR DOUBLE ROUTINES. WINDOWS PLATFORM                    *
*                                                                              *
*******************************************************************************/
...
__func__(double nan(const char *tagp))
{
  ...
  cvt.l = __host_device_call(__internal_nan_kernel (tagp));
  return cvt.d;
}
...
/*******************************************************************************
*                                                                              *
* HOST IMPLEMENTATION FOR FLOAT ROUTINES. WINDOWS PLATFORM                     *
* MAP FLOAT ROUTINES TO DOUBLE ROUTINES WHEREEVER POSSIBLE                     *
*                                                                              *
*******************************************************************************/
...
{
  return __host_device_call(__fmaf_rn(a, b, c));
}
...
__func__(float nanf(const char *tagp))
{
  ...
  cvt.i = (unsigned int)__host_device_call(__internal_nan_kernel (tagp));
