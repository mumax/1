...
func Exch6NgbrKernel(unpaddedsize []int, cellsize []float) *Tensor5{
  ...
  k.Array()[s][s][0][0][0] = -2./(cellsize[X]*cellsize[X])-2./(cellsize[Y]*cellsize[Y])-2./(cellsize[Z]*cellsize[Z]);
	...
	Set(k, index, 1./(cellsize[dir]*cellsize[dir]));
      }
    }
...
func Exch6NgbrKernelCubic(unpaddedsize []int, cubesize float) *Tensor5{
  ...
  k.Array()[s][s][0][0][0] = -6./(cubesize*cubesize);
	...
	index[1] = s;
	index[dir+2] = wrap(side, size[dir]);
	Set(k, index, 1./(cubesize*cubesize));
      }
    }
...
// func Exch6NgbrKernel(unpaddedsize []int, cubesize float) *ExchKern{
//   return &ExchKern{[]int{3, 3, unpaddedsize[0], unpaddedsize[1], unpaddedsize[2]}, cubesize};
// }
// 
// type ExchKern struct{
//   size []int;
//   cubesize float;
// }
// 
// func (k *ExchKern) Size() []int{
//   return k.size;
// }
// 
// func (k *ExchKern) Get(index []int) float{
//   Assert(len(index) == 5);
//   if iMax(iAbs(index[2]), iAbs(index[3]), iAbs(index[4])) == 1 && index[0] == index[1]{
//       return 1./(k.cubesize*k.cubesize);
//   }
//   //else:
//     return 0.;
// }
...
// }
// 
// func iMax(i, j, k int) int{
//     return iMax2(iMax2(i, j), k);
// }
// 
// func iMax2(i, j int) int{
//   if i>j {return i};
//   return j;
// }
