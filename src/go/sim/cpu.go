...
/**
 * This single file intefaces all the relevant CUDA func(d Cpu) tions with go
 * It only wraps the func(d Cpu) tions, higher level constructs and assetions
...
TODO: FFT needs to store source and dest to be compatible with fftw plans (?)
...
func(d Cpu) newFFTPlan(dataSize, logicSize []int) unsafe.Pointer{
  ...
  return unsafe.Pointer( C.new_cpuFFT3dPlan_padded( Csize, CpaddedSize ) )
}

/// unsafe FFT
func(d Cpu) fftForward(plan unsafe.Pointer, in, out unsafe.Pointer){
  C.cpuFFT3dPlan_forward((*C.gpuFFT3dPlan)(plan), (*C.float)(in), (*C.float)(out))
}


/// unsafe FFT
func(d Cpu) fftInverse(plan unsafe.Pointer, in, out unsafe.Pointer){
  C.cpuFFT3dPlan_inverse((*C.gpuFFT3dPlan)(plan), (*C.float)(in), (*C.float)(out))
}
...
func(d Cpu) memcpyOn(source, dest unsafe.Pointer, nFloats int){
  C.memcpy_cpu_to_gpu((*C.float)(source), (*C.float)(dest), C.int(nFloats))
}
