...
/**
 * @file
 * Initialization of the Biot Savart kernel for 2D simulations with invariance considered in 
 ...
 * The Kernel components are Fourier transformed and only the real parts are stored on the device. 
 * The correction factor 1/(dimensions FFTs) is included.
 * 
 * @author Ben Van de Wiele
 */
#ifndef GPU_KERNEL_BIOT_SAVART2D_H
#define GPU_KERNEL_BIOT_SAVART2D_H
...
/**
 * Returns the initialized Biot Savart 2D kernel.  The kernel is stored as a rank 2 tensor.
 * Invariance of all quantities is considered in the x-direction. The first rank contains 
 * the Kernel elements [y, z].  The second rank contains the contiguous Fourier 
 * transformed data of each element.  
 * 
 ...
 */
void gpu_kernel_biot_savart2d(int *kernelSize,     ///< Non-strided size of the kernel data
 ...
 */
void gpu_init_and_FFT_Greens_kernel_elements_biot_savart2d(float *dev_kernel,         ///< float array: list of rank 2 tensor; rank 0: yy, yz, zz parts of symmetrical Greens tensor, rank 1: all data of a Greens kernel component contiguously
 ...
 */
__global__ void _gpu_init_Greens_kernel_elements_biot_savart2d(float *dev_temp,       ///< pointer to the temporary memory space on the device to store all elements of a given Greens tensor component
 ...
 */
__device__ float _gpu_get_Greens_element_biot_savart2d(int Nkernel_Y,           ///< Non-strided size of the kernel data (y-direction)
                                                       int Nkernel_Z,           ///< Non-strided size of the kernel data (z-direction)
                                                       int co,                 ///< co1 and co2 define the requested Greens tensor component: e.g. co1=0, co2=1 defines gxy
 ...
 */
__global__ void _gpu_extract_real_parts_biot_savart2d(float *dev_kernel_array,  ///< pointer to the first kernel element of the considered tensor element
 ...
 */
void initialize_Gauss_quadrature_on_gpu_biot_savart2d(float *dev_qd_W_10,  ///< float array (10 floats) containing the 10 Gauss quadrature weights (on device)
 ...
 */
void get_Quad_Points_biot_savart2d(float *gaussQP,      ///< float array containing the requested Gauss quadrature points
