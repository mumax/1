...
void gpu_kernel_biot_savart2d(int *kernelSize, float *cellsize, int *repetition){
  ...
  // initialization Gauss quadrature points for integrations + copy to gpu ________________________
    ...
    initialize_Gauss_quadrature_on_gpu_biot_savart2d(dev_qd_W_10, dev_qd_P_10, cellSize);
  // ______________________________________________________________________________________________
  ...
  // Plan initialization of FFTs and initialization of the kernel _________________________________
    ...
    gpu_init_and_FFT_Greens_kernel_elements_biot_savart2d(dev_kernel->list, kernelSize, exchType, exchInConv, cellSize, repetition, dev_qd_P_10, dev_qd_W_10, kernel_plan);
  // _____________________________________________about:home_________________________________________________ 
...
void gpu_init_and_FFT_Greens_kernel_elements_biot_savart2d(float *dev_kernel, int *kernelSize, float *FD_cell_size, int *repetition, float *dev_qd_P_10, float *dev_qd_W_10, gpuFFT3dPlan* kernel_plan){
  ...
  // Main function operations _____________________________________________________________________
    int rank0 = 0;                                   // defines the first rank of the Biot Savart kernel: [x,y]
    for (int co=1; co<3; co++){                      // for a Biot Savart kernel component [X,Y]:
        // Put all elements in 'dev_temp1' to zero.
      gpu_zero(dev_temp1, kernelN);    
      gpu_sync();
      // Fill in the elements.
      _gpu_init_Greens_kernel_elements_biot_savart2d<<<gridsize1, blocksize1>>>(dev_temp1, kernelSize[Y], kernelSize[Z], co, FD_cell_size[Y], FD_cell_size[Z], repetition[Y], repetition[Z], dev_qd_P_10, dev_qd_W_10);
      gpu_sync();
      ...
      _gpu_extract_real_parts_biot_savart2d<<<gridsize2, blocksize2>>>(&dev_kernel[rank0*kernelStorageN/2], dev_temp2, N);
      gpu_sync();
...
__global__ void _gpu_init_Greens_kernel_elements_biot_savart2d(float *dev_temp, int Nkernel_Y, int Nkernel_Z, int co, float FD_cell_size_Y, float FD_cell_size_Z, int repetition_Y, int repetition_Z, float *dev_qd_P_10, float *dev_qd_W_10){
  ...
  int N2 = Nkernel_Z;
    ...
    dev_temp[            j*N2 +           k] = _gpu_get_Greens_element_biot_savart2d(Nkernel_Y, Nkernel_Z, co,  j,  k, FD_cell_size_Y, FD_cell_size_Z, repetition_Y, repetition_Z, dev_qd_P_10, dev_qd_W_10);
  if (j>0)
    dev_temp[(Nkernel_Y-j)*N2 +           k] = _gpu_get_Greens_element_biot_savart2d(Nkernel_Y, Nkernel_Z, co, -j,  k, FD_cell_size_Y, FD_cell_size_Z, repetition_Y, repetition_Z, dev_qd_P_10, dev_qd_W_10);
  if (k>0) 
    dev_temp[            j*N2 + Nkernel_Z-k] = _gpu_get_Greens_element_biot_savart2d(Nkernel_Y, Nkernel_Z, co,  j, -k, FD_cell_size_Y, FD_cell_size_Z, repetition_Y, repetition_Z, dev_qd_P_10, dev_qd_W_10);
  if (j>0 && k>0) 
    dev_temp[(Nkernel_Y-j)*N2 + Nkernel_Z-k] = _gpu_get_Greens_element_biot_savart2d(Nkernel_Y, Nkernel_Z, co, -j, -k, FD_cell_size_Y, FD_cell_size_Z, repetition_Y, repetition_Z, dev_qd_P_10, dev_qd_W_10);
...
__device__ float _gpu_get_Greens_element_biot_savart2d(int Nkernel_Y, int Nkernel_Z, int co, int b, int c, float FD_cell_size_Y, float FD_cell_size_Z, int repetition_Y, int repetition_Z, float *dev_qd_P_10, float *dev_qd_W_10){
  ...
  // for elements in Kernel component gy _________________________________________________________
    if (co==1){
      ...
      for(int cntc=-repetition_Z; cntc<=repetition_Z; cntc++){
        ...
        if (r2_int<400){
          ...
          result += FD_cell_size_Y * FD_cell_size_Z * ( j*FD_cell_size_Y/r2 );
        }
      }
  ...
  // for elements in Kernel component gz _________________________________________________________
    if (co==2){
      for(int cntb=-repetition_Y; cntb<=repetition_Y; cntb++)
      for(int cntc=-repetition_Z; cntc<=repetition_Z; cntc++){
        ...
        if (r2_int<400){
          ...
          float r2 = (j*FD_cell_size_Y)*(j*FD_cell_size_Y) + (k*FD_cell_size_Z)*(k*FD_cell_size_Z);
          result += FD_cell_size_Y * FD_cell_size_Z * ( k*FD_cell_size_Z/r2 );
        }
      }
      result *= 1.0f/2.0f/3.14159265f*dim_inverse;
...
__global__ void _gpu_extract_real_parts_biot_savart2d(float *dev_kernel_array, float *dev_temp, int N){
...
void initialize_Gauss_quadrature_on_gpu_biot_savart2d(float *dev_qd_W_10, float *dev_qd_P_10, float *FD_cell_size){
  ...
  // Map the standard Gauss quadrature points to the used integration boundaries __________________
    ...
    get_Quad_Points_biot_savart2d(&host_qd_P_10[ 0], std_qd_P_10, 10, -0.5f*FD_cell_size[Y], 0.5f*FD_cell_size[Y]);
    get_Quad_Points_biot_savart2d(&host_qd_P_10[10], std_qd_P_10, 10, -0.5f*FD_cell_size[Z], 0.5f*FD_cell_size[Z]);
  // ______________________________________________________________________________________________
...
void get_Quad_Points_biot_savart2d(float *gaussQP, float *stdGaussQP, int qOrder, double a, double b){
