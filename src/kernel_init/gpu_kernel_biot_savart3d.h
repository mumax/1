...
/**
 * @file
 * Initialization of the Biot Savart kernel for 3D simulations (possibly only 1 cell thickness).  
 * The Kernel components are Fourier transformed and only the real 
 * parts are stored on the device. The correction factor 1/(dimensions FFTs) is included.
 * 
 * @author Ben Van de Wiele
 */
#ifndef GPU_KERNEL_BIOT_SAVART3D_H
#define GPU_KERNEL_BIOT_SAVART3D_H
...
/**
 * Returns the initialized biot savart 3D kernel.  The kernel is stored as a rank 2 tensor.
 * For a 3D simulation with thickness > 1 FD cell, the first rank contains the Kernel elements:
 * [x, y, z], for a 3D simulation with thickness = 1 FD cell (only possible in X-direction!) 
 * the first rank contains only the non-zero elements: [y, z].
 * The second rank contains the contiguous Fourier transformed data of each element.  
 * 
 ...
 * 
 * the correction factor 1/(dimensions FFTs) are included.
 */
void gpu_kernel_biot_savart3d(int *kernelSize,     ///< Non-strided size of the kernel data
 ...
 */
void gpu_init_and_FFT_Greens_kernel_elements_biot_savart3d(float *dev_kernel,  			  ///< float array: list of rank 2 tensor; rank 0: xx, xy, xz, yy, yz, zz parts of symmetrical Greens tensor, rank 1: all data of a Greens kernel component contiguously
 ...
 */
__global__ void _gpu_init_Greens_kernel_elements_biot_savart3d(float *dev_temp, 			///< pointer to the temporary memory space on the device to store all elements of a given Greens tensor component
                                                               ...
                                                               int co, 							///< co defines the requested Greens tensor component
...
/**
 * Returns an element with coordinates [a,b,c] of the Greens kernel component defined by 'co'.
 */
__device__ float _gpu_get_Greens_element_biot_savart3d(int Nkernel_X, 					///< Non-strided size of the kernel data (x-direction)
                                                       int Nkernel_Y, 					///< Non-strided size of the kernel data (y-direction)
                                                       int Nkernel_Z, 					///< Non-strided size of the kernel data (z-direction)
                                                       int co, 								///< co defines the requested Greens tensor component
 ...
 */
__global__ void _gpu_extract_real_parts_biot_savart3d(float *dev_kernel_array, 	///< pointer to the first kernel element of the considered tensor element
 ...
 */
void initialize_Gauss_quadrature_on_gpu_biot_savart3d(float *dev_qd_W_10,  ///< float array (10 floats) containing the 10 Gauss quadrature weights (on device)
                                                      float *dev_qd_P_10,  ///< float array (30 floats) containing the 10 Gauss quadrature points for X, Y and Z contiguously (on device)
 ...
 */
void get_Quad_Points_biot_savart3d(float *gaussQP, 			///< float array containing the requested Gauss quadrature points
