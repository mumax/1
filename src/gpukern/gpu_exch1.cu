...
void gpu_add_exch (float *m, float *h, int *size, int *periodic, int *exchInconv0, float *cellSize, int type){
  ...
  for (int i=0; i<3; i++)
    if (exchInConv[i]==0){
      ...
      switch (type){
        case EXCH_6NGBR:
          if size[X] == 1)
            gpu_add_6NGBR_exchange_2D_geometry (m_comp, h_comp, int *size, int *periodic, float *cellSize);
          else 
            gpu_add_6NGBR_exchange_3D_geometry (m_comp, h_comp, int *size, int *periodic, float *cellSize);
          break;
/*        case EXCH_12NGBR:
          if size[X] == 1)
...
void gpu_add_6NGBR_exchange_3D_geometry (float *m, float *h, int *size, int *periodic, float *cellSize){
  ...
  float cst_xyz = -2.0f/cellSize[X]/p->cellSize[X] - 2.0f/cellSize[Y]/p->cellSize[Y] - 2.0f/cellSize[Z]/cellSize[Z];
...
void gpu_add_6NGBR_exchange_2D_geometry (float *m, float *h, int *size, int *periodic, float *cellSize){
  ...
  float cst_y = 1.0f/cellSize[Y]/cellSize[Y];
  float cst_z = 1.0f/cellSize[Z]/cellSize[Z];
  float cst_yz = -2.0f/cellSize[Y]/cellSize[Y] - 2.0f/cellSize[Z]/p->cellSize[Z];
