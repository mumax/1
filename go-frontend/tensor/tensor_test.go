...
func TestTensorOps(t *T){
  ...
  for i:=NewIterator(a); i.HasNext(); i.Next(){
    if a.Get(i.Index()) != 0.5 {t.Fail()};
    count++;
  }
  if count != 3*4*2 {t.Fail()};

  b := Copy(a);
  if !Equals(a,b) { t.Fail() };

  c := Buffer(a);
  if !Equals(a,c) { t.Fail() };

  // test Slicing
  s1 := Slice(a, 0, 1);
  if !reflect.DeepEqual(s1.Size(), []int{3, 4}) { t.Fail() };
  for i:=0; i<3; i++{
    for j:=0; j<4; j++{
      if a.Array()[0][i][j] != s1.Get([]int{i, j}) { t.Fail() };
    }
  }

  s2 := Slice(a, 1, 2);
  if !reflect.DeepEqual(s2.Size(), []int{2, 4}) { t.Fail() };
  for i:=0; i<2; i++{
    for j:=0; j<4; j++{
      if a.Array()[0][i][j] != s1.Get([]int{i, j}) { t.Fail() };
    }
  ...
  s3 := Slice(a, 2, 0);
  if !reflect.DeepEqual(s3.Size(), []int{2, 3}) { t.Fail() };
  s4 := Slice(s3, 0, 1);
  if !reflect.DeepEqual(s4.Size(), []int{3}) { t.Fail() };

  // test Transpose
  t1 := Transpose(a, 0, 1);
  if !reflect.DeepEqual(t1.Size(), []int{3, 2, 4}) { t.Fail() };
...
func TestTensor2(t *T){
  ...
  if Rank(a) != 2 { t.Fail() };
  if a.Size()[0] != 3 { t.Fail() };
  if a.Size()[1] != 4 { t.Fail() };

  a.List()[11] = 1.23;
  if a.Array()[2][3] != 1.23 { t.Fail() };
  if a.Get([]int {2, 3}) != 1.23 { t.Fail() };
  ...
  for i:=0; i<3; i++{
   ...
   for i:=0; i<3; i++{
    for j:=0; j<4; j++{
      if a.Get([]int{i, j}) != float(i + 2*j) { t.Fail() };
    }
  }
...
func TestTensor3(t *T){
  ...
  for i:=0; i<3; i++{
    for j:=0; j<4; j++{
      for k:=0; k<5; k++{
	if a.Get([]int{i, j, k}) != float(i + 2*j + 3*k) { t.Fail() };
      }
    }
  }
}
...
func TestTensor4(t *T){
  ...
  for i:=0; i<3; i++{
    for j:=0; j<4; j++{
      for k:=0; k<5; k++{
	for l:=0; l<6; l++{
	  if a.Get([]int{i, j, k, l}) != float(i + 2*j + 3*k + 4*l) { t.Fail() };
	 }
      }
    }
  }
...
func TestTensor5(t *T){
  ...
  for i:=0; i<3; i++{
	...
	for l:=0; l<6; l++{
	  for m:=0; m<7; m++{
	    if a.Get([]int{i, j, k, l, m}) != float(i + 2*j + 3*k + 4*l + 5*m) { t.Fail() };
	  }
	}
      }
    }
