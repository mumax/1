...
for i; do
  ...
  cmd_b='set log y; set xlabel "time(ns)"; set ylabel "max torque(unitless)"; plot '$file' using ($1*1E9):8 with lines title "tx", '$file' using ($1*1E9):9 with lines title "ty", '$file' using ($1*1E9):10 with lines title "tz";'
